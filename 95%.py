...
t_full = np.arange(1, 32)  # 时间本身就是1-31天，无需修改
t1, t2, t3, t4 = 7, 14, 19, 27  # Five-segment nodes
...
def eqpa_5seg_model(t, y, params):
    ...
    # 取消增长率下界限制，允许base_γ低于0.1
    base_α4 = max(0.1, base_α4)
    γ_coeff = [0.01, 2.0, 7.8, 0.3, 0.001];
    α4_coeff = [1.0, 0.2, 0.03, 2.0, 30.0]
    if t <= t1:
        γ = base_γ * γ_coeff[0];
...
# %% 3. Calculate Fitting Results + Time-varying Parameters + 对称置信区间
A0 = A_obs[0];
...
if cov is None:
    ...
    res_std = np.std(res)
    param_std = [res_std * 0.0005, res_std * 0.0005]
else:
    param_std = np.sqrt(np.diag(cov)) * 0.001

# 生成参数抽样
param_samples = np.random.normal(
    loc=fitted_params,
    scale=param_std,
    size=(n_samples, len(fitted_params))
)
...
for params in param_samples:
    ...
    A_pred = interp1d(sol.t, sol.y[3], kind='linear')(t_full)
    A_pred_smoothed = gaussian_filter1d(A_pred, sigma=0.9)
    A_pred_samples.append(A_pred_smoothed)

# 计算分位数
A_pred_samples = np.array(A_pred_samples)
lower_bound = np.percentile(A_pred_samples, alpha * 100, axis=0)
upper_bound = np.percentile(A_pred_samples, (1 - alpha) * 100, axis=0)

# 验证上下界合理性
print("\n置信区间验证（第10天为例）：")
print(f"  拟合线值：{A_fit_smooth[9]:.6f}")
print(f"  下界值：{lower_bound[9]:.6f}")
print(f"  上界值：{upper_bound[9]:.6f}")

# Extract Time-varying Parameters (unchanged)
...
stages = [
    ...
    ("Day 27-31 (Linear Decline Phase)", (t_full > t4) & (t_full <= 31)),
...
# %% 5. Visualization (核心修改：X轴仅显示1-31天)
...
ax1.fill_between(
    t_full, lower_bound, upper_bound,
    color='darkblue', alpha=0.25, label='95% Confidence Interval'
)
...
ax1.set_ylabel('Adult Density (individuals/m²)', fontsize=14)
ax1.set_title('(A) Soybean Pod Borer Model Fitting Results', fontsize=14, fontweight='bold', loc='left', pad=10)
# 核心修改1：X轴范围限制为1-31，隐藏0-1天
ax1.set_xlim(1, 31)
# 核心修改2：X轴刻度仅显示1-31的整数（每2天一个刻度）
ax1.set_xticks(np.arange(1, 32, 2))
handles, labels = ax1.get_legend_handles_labels()
ax1.legend(handles[:3] + handles[5:], labels[:3] + labels[5:], fontsize=12, loc='upper left', framealpha=0.9)
ax1.grid(False)

# -------------------------- Subplot (B): Developmental Stages --------------------------
ax2 = plt.subplot(2, 2, 3)
ax2.plot(t_full, E_fit_smooth, color='green', lw=3.5, linestyle='-', label='Eggs (E)')
ax2.plot(t_full, Q_fit_smooth, color='gold', lw=3.5, linestyle='--', label='Larvae (Q)')
ax2.plot(t_full, P_fit_smooth, color='magenta', lw=3.5, linestyle='-.', label='Pupae (P)')
ax2.plot(t_full, A_fit_smooth, color='darkblue', lw=3.5, linestyle=':', label='Adults (A)')
...
ax2.set_title('(B) Dynamics of Developmental Stages', fontsize=14, fontweight='bold', loc='left', pad=10)
# 核心修改3：X轴范围和刻度同步限制为1-31
ax2.set_xlim(1, 31)
ax2.set_xticks(np.arange(1, 32, 4))  # 每4天一个刻度
...
ax3_twin = ax3.twinx()
line1 = ax3.plot(t_full, γ_vals_smooth, 'red', lw=3, linestyle='-', label='Intrinsic Growth Rate $γ$')
line2 = ax3_twin.plot(t_full, α4_vals_smooth, 'darkblue', lw=3, linestyle='--', label='Adult Senescence Rate $α_4$')
ax3.set_ylabel('Intrinsic Growth Rate $γ$', fontsize=12, color='red')
ax3.tick_params(axis='y', labelcolor='red')
ax3_twin.set_ylabel('Adult Senescence Rate $α_4$', fontsize=12, color='darkblue')
...
ax3.legend(
    lines, labels,
    fontsize=10,
    loc='upper left',  # 锚点方向（可选：upper/lower + left/right/center）
    bbox_to_anchor=(0.02, 0.3),  # 核心调节坐标！(x, y)范围0-1
    framealpha=0.9
)
ax3.set_xlabel('Time (Day)', fontsize=12)
ax3.set_title('(C) Dynamics of Time-varying Parameters', fontsize=14, fontweight='bold', loc='left', pad=10)
# 核心修改4：X轴范围和刻度同步限制为1-31
ax3.set_xlim(1, 31)
ax3.set_xticks(np.arange(1, 32, 4))  # 每4天一个刻度
ax3.grid(False)

# 调整布局并保存
plt.tight_layout()
plt.subplots_adjust(hspace=0.3)
plt.savefig('Figure_4_EQPA_Model_Fitting_Final_Xaxis_Fixed.png', dpi=300, bbox_inches='tight')
plt.show()
